...
function enrichClinicWithDoctorData(clinic, doctors, slots) {
  if (clinic.hasCrown) {
    console.log('🔸 enrichClinicWithDoctorData:', clinic.name, 'featuredDoctorId:', clinic.featuredDoctorId)
  }
  ...
  if (!doctor) {
    if (clinic.hasCrown) {
      console.log('🔸 Doctor not found for clinic:', clinic.name, 'doctorId:', clinic.featuredDoctorId)
    }
    return clinic
  }
  ...
  if (clinic.hasCrown) {
    console.log('🔸 Enriched clinic:', clinic.name, 'with doctor:', enriched.availableDoctor.name, 'slots:', enriched.availableDoctor.availableSlots.length)
  }
  return enriched
}
...
export function useSlots() {
  return useQuery(
    ...
    async () => {
      ...
      const slotsData = [
        { doctorId: 1, clinicId: 1, date: "2024-01-18", dateLabel: "Сегодня, 18.06", slots: ["14:00", "15:30", "16:15"] },
        { doctorId: 2, clinicId: 3, date: "2024-01-20", dateLabel: "Пятница, 20.06", slots: ["09:00", "09:45", "10:30", "11:15"] },
        { doctorId: 3, clinicId: 4, date: "2024-01-23", dateLabel: "Понедельник, 23.06", slots: ["11:00", "13:20", "15:00", "16:40"] },
        { doctorId: 4, clinicId: 5, date: "2024-01-25", dateLabel: "Среда, 25.06", slots: ["09:30", "11:15", "14:30"] },
        { doctorId: 5, clinicId: 6, date: "2024-01-18", dateLabel: "Сегодня, 18.06", slots: ["17:30"] },
        { doctorId: 6, clinicId: 2, date: "2024-01-19", dateLabel: "Завтра, 19.06", slots: ["10:00", "12:30", "14:45"] }
      ]
...
export function useSearchClinics(query, filters) {
  ...
  console.log('🔸 useSearchClinics - clinics:', clinics?.length, 'doctors:', doctors?.length, 'slots:', slots?.length)
  console.log('🔸 Loading states - clinics:', clinicsLoading, 'doctors:', doctorsLoading, 'slots:', slotsLoading)
  if (slotsError) console.log('🔸 Slots error:', slotsError)
  
  return useQuery(
    ['searchClinics', query, filters],
    () => {
      ...
      if (doctors && slots) {
        ...
        console.log('🔸 Crown clinics:', crownClinics.length, 'enriched:', enrichedCrownClinics.length)
        
        return enriched
      }
      
      console.log('🔸 Returning filtered clinics without enrichment - doctors:', !!doctors, 'slots:', !!slots)
...
export function useServices(clinicId) {
  return useQuery(
    ...
    async () => {
      try {
        return await fetchData(`services_${clinicId}.json`)
      } catch (error) {
...
export function useTimeSlots(specialistId, date) {
  return useQuery(
    ['timeSlots', specialistId, date],
    async () => {
      try {
        return await fetchData(`slots_${specialistId}_${date}.json`)
      } catch (error) {
        console.warn(`Failed to load time slots for specialist ${specialistId} on ${date}, using mock data:`, error)
...
export function useAvailableDates(clinicId, daysAhead = 14) {
  return useQuery(
    ['availableDates', clinicId, daysAhead],
...
export function usePrefetchData() {
  ...
  const prefetchClinics = () => {
    queryClient.prefetchQuery('clinics', () => fetchData('clinics.json'))
  }

  const prefetchServices = (clinicId) => {
    queryClient.prefetchQuery(['services', clinicId], () => fetchData(`services_${clinicId}.json`))
  }

  const prefetchSpecialists = (clinicId) => {
    queryClient.prefetchQuery(['specialists', clinicId], () => fetchData(`specialists_${clinicId}.json`))
  }

  const prefetchDoctors = () => {
    queryClient.prefetchQuery('doctors', () => fetchData('doctors.json'))
  }

  const prefetchSlots = () => {
    queryClient.prefetchQuery('slots', () => fetchData('slots.json'))
  }
